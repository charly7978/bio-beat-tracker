...
/**
 * Filtro derivativo de orden fraccional (Grünwald-Letnikov).
 * Realza picos y suprrime ruido en una sola etapa.
 * alpha ∈ (0,1): orden fraccional (0.5 es un buen balance).
 * h: paso temporal (1/fs).
 */
export function fractionalDerivative(
  signal: number[],
  alpha: number = 0.5,
  h: number = 1,
): number[] {
  ...
  const out = new Array<number>(n);
  const trunc = Math.min(n - 1, 64);
  const coeffs = new Array<number>(trunc + 1);
  coeffs[0] = 1;
  for (let k = 1; k <= trunc; k++) {
    coeffs[k] = coeffs[k - 1] * (alpha - k + 1) / k;
  }
  const invH = 1 / Math.pow(h, alpha);
  for (let i = 0; i < n; i++) {
    let sum = 0;
    const lim = Math.min(i, trunc);
    for (let k = 0; k <= lim; k++) {
...
/**
 * Potencia espectral de la banda fisiológica usando Goertzel en bins clave.
 * Devuelve la energía normalizada en la banda [fMinHz, fMaxHz].
 */
export function bandPower(
  ...
  for (let s = 0; s <= steps; s++) {
    ...
    bandPowerVal += re * re + im * im;
  }
  return bandPowerVal / Math.max(1e-12, totalPower * (steps + 1));
}

/**
 * Entropía espectral de potencia normalizada [0,1].
 * Mide qué tan plana es la distribución espectral.
 * Alta entropía → ruido/poca estructura; baja entropía → componente periódico dominante.
 */
export function spectralEntropy(signal: number[], fs: number): number {
  const n = signal.length;
  if (n < 8 || fs <= 0) return 1;
  let mean = 0;
  for (let i = 0; i < n; i++) mean += signal[i];
  mean /= n;
  const centered = new Array<number>(n);
  ...
  let totalPower = 0;
  for (let b = 0; b < bins; b++) totalPower += spectrum[b];
  if (totalPower < 1e-12) return 1;
  let entropy = 0;
  for (let b = 0; b < bins; b++) {
    const p = spectrum[b] / totalPower;
