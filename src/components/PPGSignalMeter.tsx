...
const PPGSignalMeter = React.forwardRef<PPGSignalMeterHandle, PPGSignalMeterProps>(({
  ...
  useEffect(() => {
    ...
    // Sin throttle manual: rAF ya está sincronizado al VSync del display (60/90/120 Hz).
    // Comparar Date.now() contra un frameTime fijo introducía "beating" con el
    // refresh real → micro-saltos perceptibles. Delegamos totalmente a rAF.
    const render = () => {
      if (!isRunningRef.current) return;
      const canvas = canvasRef.current;
