...
function validateInput(data: unknown): { valid: boolean; error?: string; parsed?: VitalSignsInput } {
  ...
  // At least one vital sign must be present for analysis
  const hasAnyVital = d.heartRate || d.spo2 || d.systolic || d.diastolic;
  if (!hasAnyVital) {
    return { valid: false, error: "At least one vital sign required" };
  }

  const heartRate = d.heartRate ? Number(d.heartRate) : undefined;
  const spo2 = d.spo2 ? Number(d.spo2) : undefined;
  const systolic = d.systolic ? Number(d.systolic) : undefined;
  const diastolic = d.diastolic ? Number(d.diastolic) : undefined;

  if (heartRate !== undefined && (heartRate < 20 || heartRate > 300)) {
...
function buildPrompt(v: VitalSignsInput): string {
  let vitalsText = `## Datos de Signos Vitales del Paciente\n`;

  if (v.heartRate !== undefined && v.heartRate > 0) {
    vitalsText += `\n- **Frecuencia Cardíaca:** ${v.heartRate} BPM`;
  }
  if (v.spo2 !== undefined && v.spo2 > 0) {
    vitalsText += `\n- **SpO2 (Saturación de Oxígeno):** ${v.spo2}%`;
  }
  if (v.systolic !== undefined && v.diastolic !== undefined && v.systolic > 0) {
    vitalsText += `\n- **Presión Arterial:** ${v.systolic}/${v.diastolic} mmHg`;
  }
  if (v.arrhythmiaCount !== undefined && v.arrhythmiaCount > 0) {
